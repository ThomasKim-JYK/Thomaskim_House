...
def probabilistic(summary):
    ...
    if p_cube<0.15:
        ...
        result[1] = 0
        flag = True
    elif p_cylinder<0.15:
        result[2] = 0
        result[3] = 0
        flag = True
    elif p_triangle<0.15:
        result[4] = 0
        result[5] = 0
        flag = True
    max_index = []
    if flag: #only two shapes, output biggest three value's index
        for i in range(3):
            current_max_index = result.index(max(result))
            max_index.append(current_max_index)
            result[current_max_index]=0
        return max_index
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='C:/Users/Thomas/Desktop/yolov5_objects/runs/train/exp68/weights/best.pt', help='model.pt path(s)')
    ...
    parser.add_argument('--conf-thres', type=float, default=0.65, help='object confidence threshold')#0.25
